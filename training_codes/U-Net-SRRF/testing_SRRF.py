...
import sys
path = '/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in5/'
...
if __name__ == "__main__":
    ...
    batch_size = 1
    input_size= 5
    maximum_intensity_path="/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in10/Max_intensity.npy"
    SRRFDATASET = ReconsDataset(all_data_path="/media/zfq/My_Passport/0All-SRRF_data/New_training_20190829/0NPY_Dataset/Dataset/Microtubule/",
    ...
    model.cuda(cuda)
    model.load_state_dict(torch.load("/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in5/SRRF_in5_750.pkl"))
    model.eval()
...
#        if mv < 0:
#            pred = pred + abs(mv)
        io.imsave('/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in5/prediction/' +image_name[0][:-4]+ '_in.tif',    image1[0,:,:,0].detach().cpu().numpy().astype(np.uint32))
        io.imsave('/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in5/prediction/' +image_name[0][:-4]+ '_gt.tif',    image_gt[0].detach().cpu().numpy().astype(np.uint32))
        io.imsave('/home/zfq/0_lhjin_code/SRRF_backup/SRRF/Microtubule/SRRF_in5/prediction/' +image_name[0][:-4] + '_pred.tif', pred.detach().cpu().numpy().astype(np.uint32))
