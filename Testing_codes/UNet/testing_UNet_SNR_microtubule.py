...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 14 16:23:49 2019
...
class ReconsDataset(torch.utils.data.Dataset):
     ...
     def __len__(self):
        dirs_in = os.listdir(self.test_in_path)   # open the files
        return len(dirs_in)            # because one of the file is for groundtruth
...
if __name__ == "__main__":
    ...
    SRRFDATASET = ReconsDataset(test_in_path="/home/star/0_code_lhj/DL-SIM-github/TESTING_DATA/microtuble/LE_X2/",
                                transform = ToTensor(),
                                img_type = 'tif',
                                in_size = 256)
    ...
    model.cuda(cuda)
    model.load_state_dict(torch.load("/home/star/0_code_lhj/DL-SIM-github/MODELS/UNet_SNR_microtubule.pkl"))
    model.eval()
    ...
    for batch_idx, items in enumerate(test_dataloader):
        ...
        img_type = 'tif'
        filepath = os.path.join('/home/star/0_code_lhj/DL-SIM-github/Testing_codes/UNet/prediction/'+image_name[0]+'_pred')
        train_in_size = 15
        
        for image_num in range(train_in_size):
            max_out = 5315.0
            image = pred[0,image_num]*max_out
