...
./microtubule/Sample_x.npy = [data[0]['Name'] = 'HER_SRRF_320*329'
...
class CropImages(object):
    ...
    def __init__(self, in_size,train_in_size,
                 train_in_data_path = '/media/zfq/My_Passport/0All-SRRF_data/New_training_20190829/0NPY_Dataset/Samples/Microtubule/Samples/',
                 data_crop_npy_path="/media/zfq/My_Passport/0All-SRRF_data/New_training_20190829/0Dataset/Training_testing_NPY/microtubule/",
                 img_type="tif",
                 input_data_length=42):
        
        self.in_size = in_size # 320*320
                ...
                for j in range(Num_y):
                    ...
                    if I.flatten().max() > 500:
                        ...
                        for image_num in range(train_in_size):
                            ...
                            image_t = io.imread(image_name)
#                            I = transform.resize(I,(int(in_size*5), int(in_size*5)),preserve_range=True,order=3)
                            I = image_t[i_top:i_bottom, j_left:j_right]
...
#                            I = transform.resize(I,(int(in_size*5), int(in_size*5)),preserve_range=True,order=3)
        ...
        data[2]['Name'] = 'HE_max'
        data[3]['Name'] = 'LE_max'
        np.save('/home/zfq/0_lhjin_code/SRRF/Microtubule_max_intensity.npy', data)

if __name__ == "__main__":
    mycrop = CropImages(320,20)    # to crop big image into small pieces
    mycrop.doCropImages()
